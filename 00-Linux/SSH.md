...
所谓"公钥登录"，原理很简单，就是用户将自己的`公钥储存在远程主机上`。登录的时候，远程主机会向用户发送一段随机字符串，用户用自己的`私钥`加密后，再发回来。远程主机用事先储存的公钥进行解密，如果成功，就证明用户是可信的，直接允许登录shell，不再要求密码。
...
运行结束以后，在$HOME/.ssh/目录下，会新生成两个文件：id_rsa.pub`公钥`和id_rsa`私钥`。前者是你的`公钥`，后者是你的`私钥`。
