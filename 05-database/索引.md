...
1. 最适合索引的列是出现在 `WHERE子句中`的`列`，或连接子句中的列而不是出现在 SELECT关键字后的列
2. 索引列的基数越大，索引的效果越好
...
6. 列类型是`字符串`，查询时一定要给值`加引号`，否则索引失效
...
![img](https://meross-static-file.oss-cn-shenzhen.aliyuncs.com/uploads/1609923929944/1/9944/12/pic/20210310164035.png)
