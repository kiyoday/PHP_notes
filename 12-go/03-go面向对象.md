> 在讲解 Go 语言面向对象内容之前，需要说明下 Go 语言的代码是以`包结构`来组织的，且如果标示符（变量名，函数名，自定义类型等）如果以大写字母开头那么这些标示符是可以导出的，可以在任何导入了定义该标示符的包的包中直接使用。Go 语言中的面向对象和 C++，Java 中的面向对象不同，因为 Go 语言`不支持继承`，Go 语言只支持`组合`。
